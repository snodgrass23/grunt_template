beforeEach(function() {
  this.addMatchers({
    ...
    toHaveArgsForCall: function(callIndex, expectedArgs) {
      if (callIndex<0) callIndex = this.actual.calls.length + callIndex;

      for (var i = 0; i < expectedArgs.length; i++) {
        if (this.actual.argsForCall[callIndex][i] !== expectedArgs[i]) {
          return false;
        }
      }
